...
public class LLMAnalyzer {
    ...
    public ChamberSpecList analyze(RawBundle bundle) {
        ...
        log.info("Грузооборот: {}", volumes);
        log.info("Габариты: {}", dimensions);
        log.info("Смежные помещения: {}", adjacent);
        log.info("Продукция: {}", product);
        log.info("Персонал и оборудование: {}", heat);
        log.info("Вентиляция: {}", vent);
        log.info("Ограждающие конструкции: {}", insul);
        log.info("Двери и ворота: {}", doors);
