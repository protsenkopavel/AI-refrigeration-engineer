...
public class LLMAnalyzer {
    ...
    ValidationResult validate(String freeFormText, ChamberSpecList specList) {
        ...
        String prompt = """
                ...
                - Перепутанные помещения: значения присвоены не тому помещению (например, площади перепутаны)
                - Подозрительные паттерны: одинаковое значение у всех камер (например, одна высота для всех)
                  без явного обоснования в тексте
                - Потерянные инженерные заметки: в тексте есть важные требования, не попавшие ни в
                  engineeringNotes, ни в globalRequirements
                - Потерянные позиции персонала/оборудования: в тексте перечислены роли, а в JSON их нет
                
                Для каждого расхождения укажи: камеру (или "global"), поле, значение в тексте,
                значение в JSON, и рекомендацию по исправлению.
                
                Также добавь warnings для:
                - Значений с confidence LOW/INFERRED, которые стоит перепроверить
                - Полей, где null, но данные можно было бы вывести из контекста
                
                valid = true только если расхождений нет.
